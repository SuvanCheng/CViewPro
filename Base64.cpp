...
std::string Base64::base64_encode(unsigned char const* bytes_to_encode, unsigned int in_len)
{
	...
	unsigned int nBlock = in_len/3;
	if(in_len%3>0){
		nBlock += 1;
	}


	unsigned char* pBuffer = (unsigned char*)malloc(4*nBlock+1);
	if(pBuffer==NULL){
		return "";
	}
	memset(pBuffer,0,4*nBlock+1);
	unsigned int nPos = 0;
	...
	while (in_len--) {
		...
		if (i == 3) {
			...
			for(i = 0; (i <4) ; i++){
				...
				pBuffer[nPos] = (unsigned char)base64_chars[char_array_4[i]];
				nPos++;
			}
	...
	if (i)
	{
		...
		for (j = 0; (j < i + 1); j++){
			...
			pBuffer[nPos] = (unsigned char)base64_chars[char_array_4[j]];
			nPos++;
		}
	...
	ret.assign((char*)pBuffer,4*nBlock);
	free(pBuffer);
