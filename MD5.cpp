...
CString CMd5::MDString (char *in)
{
	unsigned int len = strlen(in);
	return MDString(in,len);
}

CString CMd5::MDString (char *in,unsigned int len)
{
	MD5_CTX context;
	...
	for(int i = 0; i < 16; i++){
		//sprintf((char*)(output+2*i),"%02x",(unsigned char)digest[i]);
		sprintf_s((char*)(output+2*i),4,"%02x",(unsigned char)digest[i]);
	}
	CString sResult = (char*)output;
	return sResult;
}


CString CMd5::MDFile (CString filename)
{ 
