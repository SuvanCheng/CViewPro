...
BOOL CTwain::InitTwain(HWND hWnd)
{
	...
	m_hMessageWnd = hWnd;
//	_tcscpy(libName,_T("TWAIN_32.DLL"));//2.0 TWAINDSM.DLL
	_tcscpy(libName,_T("TWAINDSM.DLL"));//2.0 TWAINDSM.DLL
	...
	if(IsValidDriver())
	{
		...
		if((m_AppId.SupportedGroups & DF_DSM2) == DF_DSM2)
		{
            ...
            CallTwainProc(&m_AppId,NULL,DG_CONTROL,DAT_ENTRYPOINT,MSG_GET,(pTW_ENTRYPOINT)&m_DSM_Entry)
            */
        }        
...
void CTwain::GetIdentity()
{
	...
	m_AppId.SupportedGroups |= DF_APP2;

	strcpy (m_AppId.Manufacturer, "CLTPro Working Group");
	strcpy (m_AppId.ProductFamily, "CLTPro");
	strcpy (m_AppId.ProductName, "CLTPro");
